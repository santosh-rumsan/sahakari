...
import { useDialogPortalContainer } from "@rs/ui/dialog";
import {
  InputGroup,
  InputGroupAddon,
  InputGroupButton,
  InputGroupInput,
} from "@rs/ui/input-group";
...
function Combobox<Value, Multiple extends boolean | undefined = false>({
  ...
  const itemToStringLabel =
    ...
    (defaultItemToStringLabel as (item: Value) => string);
  const itemToStringValue =
    itemToStringValueProp ??
    (defaultItemToStringValue as (item: Value) => string);

  const resolvedFilter = React.useMemo(() => {
    if (filter) return filter;
    if (!filterMode) return undefined;
    return (item: Value, query: string) => {
  ...
  }, [filter, filterMode, itemToStringLabel, itemToStringValue]);
...
function ComboboxContent({
  ...
  const resolvedPortalContainer =
    portalContainer ?? dialogPortalContainer ?? undefined;

  return (
    <ComboboxPrimitive.Portal container={resolvedPortalContainer}>
